...
const myBinaryFile = new BinaryFile('C:/Program Files (x86)/Steam/steamapps/common/Star Wars Episode I Racer/data/lev01/out_spriteblock.bin', 'w');
...
async function getReplacements() {
    ...
    for (let s = 0; s < 179; s++) {
        ...
        } else if (fs.existsSync('sprites/' + s + '.png')) {
            replacements[s] = sprites.sprites[s]
        }
    }
    const result = await Promise.all(replacements)
    return replacements
...
getReplacements().then(async function (replacements) {
    ...
    myBinaryFile.open().then(function () {
        ...
        await file.writeUInt32(total, position = cursor) //write total number of textures
        cursor += total * 4 + 8
        ...
        async function writeSprite(cursor, index) {
            ...
            rep.offset = cursor
            await file.writeUInt32(cursor, position = (4 + index * 4)).then(async () => { //write initial pointer

                //write header
                await file.writeUInt16(rep.width, position = cursor)
                cursor += 2
                await file.writeUInt16(rep.height, position = cursor)
                cursor += 2
                await file.writeUInt16(rep.format, position = cursor)
                cursor += 4
                rep.palette_offset = cursor
                cursor += 4
                await file.writeUInt16(rep.page_count, position = cursor)
                cursor += 2
                await file.writeUInt16(32, position = cursor)
                cursor += 2
                await file.writeUInt32(20, position = cursor)
                cursor += 4

                //write page info
                for (let p = 0; p < rep.pages.length; p++) {
                    let page = rep.pages[p]
                    await file.writeUInt16(page.width, position = cursor)
                    cursor += 2
                    await file.writeUInt16(page.height, position = cursor)
                    cursor += 2
                ...
                if ([512, 513].includes(rep.format)) {
                    await file.writeUInt32(cursor - rep.offset, position = rep.palette_offset) //write palette offset
                    for (let j = 0; j < rep.palette.length; j++) {
                        ...
                        let pal = (((r | g) | b) | a)
                        await file.writeUInt16(pal, position = cursor).then(async () => {
                            cursor += 2
                        })
                    }
                }
                ...
                for (let p = 0; p < rep.pages.length; p++) {
                    let page = rep.pages[p]
                    await file.writeUInt32(cursor - rep.offset, position = page.offset) // write page offset
                    if ([512, 1024].includes(rep.format)) {
                        for (let i = 0; i < page.pixels.length / 2; i++) {
                            if (rep.format == 512) {
                                ...
                                await file.writeUInt8(pixel, position = cursor).then(() => {
                                    cursor += 1
                                })
                        ...
                        for (let i = 0; i < page.pixels.length; i++) {
                            ...
                            if (rep.format == 3) {
                                for (let j = 0; j < 4; j++) {
                                    await file.writeUInt8((page.pixels[i][j]), position = cursor)
                                    cursor += 1
                                }
                            } else {
                                await file.writeUInt8(pixel, position = cursor)
                                cursor += 1
                            }
                        }
                    }
                ...
                if (index < Object.keys(replacements).length - 1) {
                    ...
                    await file.writeUInt32(cursor, position = (4 + total * 8))
                }
