...
const file = fs.readFileSync('./in/n64/n64_textureblock.bin')
texdata = {
    "0": {
        "format": 512,
        "width": 64,
...
for (let j = 0; j < out_texture.texture_count; j++) {
    ...
    if (j == out_texture.texture_count - 1) {
        end = file.readInt32BE(cursor + 8)
    }
}

for (t = 0; t < Object.keys(texdata).length - 1; t++) {
    tex = texdata[t]
    ...
    */
    ...
    if (tex.palette_offset !== 0) {
        for (let i = tex.palette_offset; i < end && i < texdata[t + 1].pixels_offset; i += 2) {
...
fs.writeFile("textures/texdata_n64.json", JSON.stringify(texdata), (err) => {
            ...
            let img = new Jimp(tex.width, tex.height, (err, image) => {
                ...
                for (let i = 0; i < tex.height; i++) {
                    for (j = 0; j < tex.width; j++) {
                        ...
                        //console.log(color, j, tex.height - 1 - i)
                        let x = 0
                        if (i % 2 == 1 && [49, 58, 68, 80, 99, 924, 966, 972, 991, 992, 1000, 1014, 1048, 1064, 1130, 1134].includes(ind)) {
                ...
                img.write('textures/' + ind + '_n64.png', (err) => {

                    if (err) console.log(ind, err);
                });
            })
        }
    }
