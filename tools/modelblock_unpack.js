...
for (let j = 0; j < out_block.addresses.length - 1; j++) {
  ...
  let highlights = []
  for (let i = out_block.addresses[j]; i < out_block.addresses[j + 1]; i++) {
    let push = file.readUInt8(i)
    let bin = (push).toString(2)
    bin = "00000000".substr(bin.length) + bin
    for (let b = 0; b < 8; b++) {
      highlights.push(bin[b])
    }
  }
  ...
  let local_offset = 0
  let modelstrings = []
  let modelstring = "string, float, int32, int16_1, int16_2, int8_1, int8_2, int8_3, int8_4, offset, pointer"
  modelstrings.push(modelstring)
  for (let i = cursor; i < out_block.addresses[j + 1]; i += 4) {
    data[i] = {}
    cursor = i
    data[i].pointer = highlights[(cursor - data.offset) / 4]
    ...
    cursor++
    local_offset += 4
    modelstrings.push([data[i].string, data[i].float, data[i].int32, data[i].int16_1, data[i].int16_2, data[i].int8_1, data[i].int8_2, data[i].int8_3, data[i].int8_4, data[i].offset, data[i].pointer].join(", "))
  }

  // this block of code offers a way to dump every possible value of a model's binary
  let name = Math.floor(j / 2)
  fs.writeFile("dump/" + name + "_" + out_block.addresses[j] + ".csv", modelstrings.join('\n'), (err) => {
                ...
                if (repeattracker.includes(address) && address !== 0) { //repeat child
                  ...
                  if (mesh_group) {
                    ...
                    if (header.visuals.index_buffer) {
                      ...
                      let v = 0
                      if (model.extension == 'Trak') {
                        //console.log((index_buffer + v) * 4, ((index_buffer + v) * 4) % 8)
                      }
  ...
  fs.writeFile(test ? ("test/" + model.model + ".json") : ("models/" + model.model + ".json"), JSON.stringify(model), (err) => {
    if (err) console.error(err)
  })
